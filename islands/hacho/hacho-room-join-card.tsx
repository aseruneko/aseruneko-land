...
export default function (props: { roomId: string }) {
  ...
  async function onClickJoinButton() {
    ...
    await post(`../api/hacho/hachoes/${props.roomId}/join`, {
      userName: userName.value,
      password: password.value.length > 0 ? password.value : undefined,
    }).then((res) => {
      if (res.joined) {
        submitting.value = false;
        window.localStorage.setItem("hacho-userId", res.userId);
        window.localStorage.setItem("hacho-userName", userName.value);
        if (password.value.length > 0) {
          window.localStorage.setItem("hacho-password", password.value);
        }
      } else {
        submitting.value = false;
