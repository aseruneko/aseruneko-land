export class Hacho {
  ...
  join(
    ...
    if (
      this.password
        ...
        : true && this.status == HachoStatus.Waiting
    ) {
  ...
  start(): boolean {
    if (this.status == HachoStatus.Waiting) {
      this.status = HachoStatus.Ongoing;
      new Array(this.maxRound).fill("").map((_, idx) => {
        const label = Hacho.prepareLabels().split(",");
        this.rounds.push(
          {
            picker: this.users[idx % this.users.length],
            minLabel: label[0],
            maxLabel: label[1],
  ...
  guess(guess: number): boolean {
    ...
    if (!round.isSubmitted) {
      this.rounds[this.round].guess = guess;
      return true;
    } else return false;
  }
  submit(): boolean {
    const round = this.rounds[this.round];
    if (round.guess != undefined) {
      this.rounds[this.round].isSubmitted = true;
      this.rounds[this.round].point = Hacho.calcPoint(
        round.correct,
        round.guess ?? 0,
      );
      this.point = this.point + this.rounds[this.round].point;
      return true;
    } else return false;
  }
  next(): boolean {
    ...
    if (round.isSubmitted) {
      if (this.round + 1 >= this.maxRound) {
        this.status = HachoStatus.Finished;
      } else {
